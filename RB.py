...
GOLDENEYE_BANNER = 'RB  a etais crée par RPZ & BLACKLOBBY'
...
class GoldenEye(object):
    ...
    def fire(self):
        ...
        self.printHeader()
        print("lancer en mode '{0}' with {1} threads {2} conection comment a attaquer CTRL+C pour stoper.".format(self.method, self.nr_workers, self.nr_sockets))

        if DEBUG:
            print("demarrage {0} concurrent threads".format(self.nr_workers))
    ...
    def monitor(self):
        while len(self.workersQueue) > 0:
            try:
                ...
                print("CTRL+C pour stoper")
                for worker in self.workersQueue:
                    try:
                        if DEBUG:
                            print("stop threads {0}".format(worker.name))
                        #worker.terminate()
                        worker.stop()
                    except Exception:
                        pass # silently ignore
...
class Striker(Process):
    ...
    def run(self):
        ...
        if DEBUG:
            print("lancement des threads {0}".format(self.name))
        ...
        if DEBUG:
            print("threads {0} stoper".format(self.name))
...
def usage():
    ...
    print(GOLDENEYE_BANNER)
    print()
    print(' USAGE: python3 RB.py <url> [OPTIONS]')
    print()
    print(' OPTIONS:')
    print('\t Flag\t\t\tDescription\t\t\t\t\t\tDefault')
    print('\t -u, --useragents\tpour utiliser les useragents\t\t\t\t(default: generation aleatoire)')
    print('\t -w, --workers\t\tNumber of threads\t\t\t\t(default: {0})'.format(DEFAULT_WORKERS))
    print('\t -s, --sockets\t\tNumber of Connection\t\t\t\t(default: {0})'.format(DEFAULT_SOCKETS))
